...
EventTypes.ROUND_NUMBER_CHANGED = 'roundNumber:changed';
...
DataManager.prototype.getCurrentMatchNumber = function() {
    return this.totalMatches + 1;
}
...
LocalizationManager.prototype.getCurrentLocale = function () {
    return LocalizationManager._currentLocale || "en";
};
...
class ScreenMainMenu extends BaseWindow {
    ...
    onTapToContinuePressed() {
        APIMediator.showInterstitialAd('button:mainmenu:startTournament', 'start').then(() => {
    ...
    async _updateDailyChallengeButton() {
        ...
        const hasRewardedVideo = APIMediator.isRewardedAdAvailable('button:dailychallenge:start');
...
class ScreenRoundSearch extends BaseWindow {
    ...
    _onShow() {
        ...
        /* set their data */
        this.playerName.element.text = DataManager.getInstance().username || LocalizationManager.getInstance().getLocalizedText("You");
...
class Round {
    ...
    doAnswerCorrect(contestantType, answerText, points, hasKey) {
        if (contestantType === Constants.Contestants.PLAYER) {
            ...
            APIMediator.sendScore(this.playerScore * this.multiplier + RoundManager.getInstance().getPlayersCompletedRoundsScore());
...
class PopupAnswersRecap extends BaseWindow {
    ...
    _onShow() {
        ...
        /* show round name */
        this.levelText.element.text = `--- ` + LocalizationManager.getInstance().getLocalizedText(`ROUND ${RoundManager.getInstance().roundIndex}`) + ` ---`;
...
MatchManager.prototype.startMatch = async function () {
    ...
    await APIMediator.gameStart(DataManager.getInstance().getCurrentMatchNumber());
...
MatchManager.prototype.showHintAndProceedToRound = async function () {
    if (RoundManager.getInstance().getRoundIndex() !== 1) {
        ...
        const rewardedAvailable = APIMediator.isRewardedAdAvailable('button:hint:get');
        if (rewardedAvailable || APIMediator.areRewardedHintsDisabled()) {
...
MatchManager.prototype._spawnPlayer = function () {
    ...
    this.playerContestantData = new ContestantData(
        ...
        DataManager.getInstance().username || LocalizationManager.getInstance().getLocalizedText("You"),
        playerColors,
...
class ScreenClaimReward extends BaseWindow {
    ...
    _onShow() {
        ...
        this._rewardedFeaturesActivated = false;
    ...
    update(dt) {
        ...
        /* buttons visibility */
        this.buttonsContainer.setLocalPosition(this._buttonsContainerOriginalPosition);
        if (APIMediator.isRewardedAdAvailable('button:victory:multiplyreward') && !this._rewardClaimed) {
            this.rewardedButton.enabled = true;
            this.rewardedButton.setAvailable(true);
            this.claimButton.setLocalPosition(pc.Vec3.ZERO);
            this.rewardedMultiplierGauge.enabled = true;

            if (!this._rewardedFeaturesActivated) {
                this._rewardedFeaturesActivated = true;
            ...
            this.rewardedButton.enabled = false;
...
class PopupSettings extends BaseWindow {
    ...
    updateButtonsVisibility() {
        ...
        this.privacyPolicyContainer.enabled = APIMediator.isPrivacyPolicyEnabled();
...
ShopSkinsTab.prototype._show = function () {
    ...
    this._hasRewardedVideo = APIMediator.isRewardedAdAvailable('button:shop:premiumskin');
    this._updateSkins();
};
...
ShopSkinsTab.prototype.onSkinClicked = function (skinName) {
    ...
    if (skinUnlocked) {
        ...
        if (skinRewarded && APIMediator.areRewardedAdsSupported()) {
            if (APIMediator.isRewardedAdAvailable('button:shop:premiumskin')) {
...
ShopSkinsTab.prototype._updateButtonsVisibility = function () {
    if (DataManager.getInstance().money >= DataManager.getInstance().nextRandomSkinPrice && !SkinManager.getInstance().areAllDefaultSkinsUnlocked()) {
        ...
        this._hasRewardedVideo = APIMediator.isRewardedAdAvailable('button:shop:freemoney');
    }
};
...
ShopScenesTab.prototype.update = function (dt) {
    ...
    this._hasRewardedVideo = APIMediator.isRewardedAdAvailable('button:shop:unlockscene');
    this._updateButtonsVisibility();
};
...
class ScreenChestroom extends BaseWindow {
    ...
    _updateButtonsVisibility() {
        const rewardedAvailable = APIMediator.isRewardedAdAvailable('button:chestroom:getfreekeys');
        if (DataManager.getInstance().keys > 0) {
            this._showFooterKeysGroup();
        } else if (DataManager.getInstance().keys === 0 && this._openedChests < this.chests.length && rewardedAvailable) {
...
class PopupHint extends BaseWindow {
    ...
    async _onYesPressed() {
        ...
        const result = await APIMediator.watchRewardedVideo('button:hint:get');
        if (result) {
            RoundManager.getInstance().getActiveRound().enableHint();
            this.app.fire(EventTypes.HIDE_LOCATION, Constants.Locations.HINT);
            UIController.getInstance().showWindow(Constants.Screens.ROUND);
    ...
    update(dt) {
        if (!DataManager.getInstance().freeHintClaimed || APIMediator.areRewardedHintsDisabled() || !APIMediator.isRewardedAdAvailable('button:hint:get')) {
...
class PopupPuzzle extends BaseWindow {
    ...
    _updateButtonsVisibility() {
        const hasRewardedAd = APIMediator.isRewardedAdAvailable('button:puzzles:unlock');
        if (hasRewardedAd) {
...
class PopupDailyChallengePresenter extends BaseWindow {
    ...
    _updateButtonsVisibility() {
        const rewardedPresent = APIMediator.isRewardedAdAvailable('button:dailychallenge:start');
        this.rewardedPlayButton.enabled = rewardedPresent;
        this.freePlayButton.enabled = !rewardedPresent;
    }
...
class PopupDailyChallengeDefeat extends BaseWindow {
    ...
    update(dt) {
        if (this.buttonsContainer.enabled) {
            if (APIMediator.isRewardedAdAvailable('button:dailychallenge:restart')) {
...
class PopupDailyChallengeVictory extends BaseWindow {
    ...
    initialize() {
        ...
        this.noThanksText = this.buttonNoThanks.findByName('NoThanksText');
    ...
    _onShow() {
        ...
        this.noThanksText.element.key = 'No thanks';
    ...
    _onAppeared() {
        ...
        const rewardedPresent = APIMediator.isRewardedAdAvailable('button:dailychallenge:doublereward');
        if (rewardedPresent) {
    ...
    async _onYesRewardedPressed() {
        ...
        const result = await APIMediator.watchRewardedVideo('button:dailychallenge:replay');
        if (result) {
    ...
    update(dt) {
        if (this.doubleOfferContainer.enabled) {
            if (!this._rewardClaimed) {
                const rewardedPresent = APIMediator.isRewardedAdAvailable('button:dailychallenge:doublereward');
                if (rewardedPresent) {
                    this.buttonX2Rewarded.enabled = true;
                    this.noThanksText.element.key = 'No thanks';
                } else {
                    this.buttonX2Rewarded.enabled = false;
                    this.noThanksText.element.key = 'Tap to continue';
                }
            }
        }
    ...
    _updateButtonsVisibility() {
        const rewardedPresent = APIMediator.isRewardedAdAvailable('button:dailychallenge:replay');
        this.yesButtonRewarded.enabled = rewardedPresent;
        this.yesButtonFree.enabled = !rewardedPresent;
    }
}
...
class ScreenPunishments extends BaseWindow {
    ...
    _updateButtonsVisibility() {
        const rewardedPresent = APIMediator.isRewardedAdAvailable('button:punishments:punish');
        this.buttonPushRewarded.enabled = rewardedPresent;
        this.buttonPushFree.enabled = !rewardedPresent;
    }
...
class ScreenTournamentDefeat extends BaseWindow {
    ...
    _updateButtonsVisibility() {
        ...
        const rewardedAvailable = APIMediator.isRewardedAdAvailable('button:tournament:revive');
        const reviveAvailable = rewardedAvailable && !TournamentManager.getInstance().isLastMatch();
...
class ScreenTournamentClaimPrize extends BaseWindow {
    ...
    _onAppeared() {
        ...
        const rewardedPresent = APIMediator.isRewardedAdAvailable('button:tournament:doublereward');
        if (rewardedPresent) {
            this.doubleOfferContainer.enabled = true;
            this.doubleOfferContainer.fire(EventTypes.UI_ELEMENT.APPEAR_MANUALLY);
        } else {
...
class ScreenTournamentStart extends BaseWindow {
    ...
    _onShow() {
        ...
        this.opponentName.element.text =  DataManager.getInstance().opponentName || LocalizationManager.getInstance().getLocalizedText("You");
        this.opponentName.element.color = new pc.Color().copy(opponentData.color.UI);
        this.app.fire(EventTypes.SET_CONTESTANT_COLORS, Constants.Contestants.OPPONENT, opponentData.color);
...
// FamobiLogo.js
var FamobiLogo = pc.createScript('famobiLogo');

FamobiLogo.prototype.initialize = function () {

    this.entity.element.opacity = 0.0;

    if (window.GameInterface && window.GameInterface.hasFeature('copyright')) {
        this.app.loader.getHandler("texture").crossOrigin = "anonymous";

        const asset = new pc.Asset("famobiLogo", "texture", {
            url: window.GameInterface.getCopyrightLogoURL()
        });

        this.app.assets.add(asset);

        asset.on("error", (message) => {
            famobi.log("Famobi logo has failed to load:", message);
        });
...
FamobiLogo.prototype.update = function (dt) {

};


// localizedLogo.js
var LocalizedLogo = pc.createScript('localizedLogo');

LocalizedLogo.prototype.initialize = function () {
    this.app.on('app:changeLocale', this._updateLogoSource, this);
};

LocalizedLogo.prototype.postInitialize = function () {
    this._updateLogoSource();
};

LocalizedLogo.prototype._updateLogoSource = function () {
    const language = LocalizationManager.getInstance().getCountryCode();
    if (language !== "en") {
        const localizedAsset = this.app.assets.find('logo_' + language + '.webp', 'texture');
        const fallbackAsset = this.app.assets.find('logo.webp', 'texture');
        if (localizedAsset) {
            AssetsLoader.getInstance().loadAsset(localizedAsset).then(() => {
                this.entity.element.textureAsset = localizedAsset.id;
            })
        } else {
            this.entity.element.textureAsset = fallbackAsset.id;
        }
    } else {
        const localizedAsset = this.app.assets.find('logo.webp', 'texture');
        this.entity.element.textureAsset = localizedAsset.id;
    }

}

LocalizedLogo.prototype.update = function (dt) {
