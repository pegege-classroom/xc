class APIMediator {
    ...
    static areRewardedHintsDisabled() {
        return !window.GameInterface.hasFeature('rewarded_hint');
    }
    ...
    static async hasRewardedVideo(eventID) {
        if (!eventID) console.warn('no event ID set in hasRewardedVideo() call');
    ...
    static watchRewardedVideo(eventID) {
        if (!eventID) console.warn('no event ID set in watchRewardedVideo() call');
